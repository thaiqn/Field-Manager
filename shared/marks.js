...
// Canonical unit everywhere: meters (number). Ported from the Field Events
// Live design prototype; shared by the server (ranking, export) and the
// React client (boards, pads).

const M_PER_IN = 0.0254;

// feet+inches -> meters
export function fi(ft, inch) { return +(((ft * 12) + inch) * M_PER_IN).toFixed(4); }
export function toInches(m) { return m / M_PER_IN; }

// Field-event rule: imperial marks read to the lesser quarter inch.
export function fmtImperial(m) {
  if (m == null) return '';
...
export function fmtMetric(m) { return m == null ? '' : m.toFixed(2); }
export function fmtMark(m, units) { return units === 'metric' ? fmtMetric(m) : fmtImperial(m); }
export function unitSuffix(units) { return units === 'metric' ? 'm' : 'ft-in'; }

// ── Horizontal events (jumps for distance + throws) ──────────
// attempt entry: {t:'mark', v:meters} | {t:'foul'} | {t:'pass'} | null (pending)

export function sortedMarks(attempts) {
...
export function rankHorizontal(athletes, marks) {
...
export function horizOrder(flight, round) {
...
export function horizNextUp(flight) {
...
export function horizCurRound(flight) {
  const nu = horizNextUp(flight);
  return nu ? nu.round : flight.rounds - 1;
}
export function flightStarted(flight) {
...
export function vertStats(heights, rec) {
...
export function isEliminated(heights, rec) {
  let seq = '';
  heights.forEach((h, i) => { seq += (rec[i] || '').replace(/[^XO]/g, ''); });
  return /XXX/.test(seq);
}
export function rankVertical(athletes, heights, attempts) {
  const rows = athletes.map((a, i) => {
...
export function vertNextUp(flight) {
