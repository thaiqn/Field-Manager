// hytek-export.js — results export in Hy-Tek's semi-colon delimited format.
// Imports into Track & Field Meet Manager via
// File / Import / Semi-Colon Delimited Rosters/Entries File.
// Spec: H header record + one 33-field E record per individual field-event result.
...
(function (FEL) {
  ...
  function buildResults(state, units) {
    const lines = [];
    lines.push(['H', clean(state.meet.name, 45), hyDate(state.meet.date), hyDate(state.meet.date),
      'Field Events Live (prototype)', hyDate(new Date().toDateString()),
      clean(state.meet.venue, 100)].join(';'));

    state.events.forEach((ev, ei) => {
      const evNum = ei + 1;
      if (ev.kind === 'horizontal') {
        // union ranking across flights for overall place
        const union = [];
        const unionMarks = {};
        ...
        ev.flights.forEach((fl, fi) => {
          M.rankHorizontal(fl.athletes, fl.marks).forEach(r => {
            const attempted = (fl.marks[r.athlete.id] || []).some(e => e);
            if (!attempted) return; // no result to report
            r.overallPlace = overallPlace[r.athlete.id];
            lines.push(eRecord({ evNum, ev, flightIdx: fi, flightName: fl.name, row: r, units }));
          });
        });
      } else {
        ev.flights.forEach((fl, fi) => {
  ...
  function download(state, units) {
    const text = buildResults(state, units);
    const blob = new Blob([text], { type: 'text/plain' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = state.meet.name.replace(/[^\w]+/g, '-').toLowerCase() + '-hytek-results.txt';
  ...
  FEL.hytek = { buildResults, download, hyMark, eventCode };
})(window.FEL);
